...
ezra({
  ...
  try {
    // Caption for the screenshot
    const cap = `*Screenshot take by Njabulo Jb*`;
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please insert the image URL and Njabulo-Jb will extract the text for you.");
    }
...
ezra({
  ...
  try {
    if (!args || args.length === 0) {
      return sendMessage("Please describe your image and Njabulo-Jb will generate it.");
    }
    const prompt = args.join(" ");
