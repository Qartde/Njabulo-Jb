...
async function loading(dest, zk) {
  const lod = [
    ...
    "⬜⬜⬜⬜⬛⬛⬛⬛80%",
    "⬛⬛⬜⬜⬜⬜⬛⬛100%",
    "*ɴᴊᴀʙᴜʟᴏ-ᴊʙ sᴘᴇᴇᴅ ᴛᴇsᴛ ᴏʀɪɢɪɴᴀᴛᴇᴅ ғʀᴏᴍ ᴛʜᴇ sᴀᴠᴇʀ*"
  ];

  let { key } = await zk.sendMessage(dest, { text: 'Njabulo Jb speed test Loading!!! Please Wait' });
...
keith({
  ...
  const audioMessage = {
    ...
    contextInfo: {
      externalAdReply: {
        title: 'ɴᴊᴀʙᴜʟᴏ-ᴊʙ',
        body: "Hallo how are you" ,
        thumbnailUrl: "https://files.catbox.moe/70u16l.jpg" ,
        sourceUrl:  'https://whatsapp.com/channel/0029VarYP5iAInPtfQ8fRb2T' , // Corrected variable name
        mediaType: 1,
...
keith({
  ...
  try {
    ...
    await repondre("> *ɴᴊᴀʙᴜʟᴏ-ᴊʙ is Restarting from the server...*");
...
keith({
  ...
  // Check if the command is issued by the owner
  if (!superUser) {
    return repondre("*This command is restricted to the bot owner or ɴᴊᴀʙᴜʟᴏ-ᴊʙh*");
  }
...
keith(
  ...
  async (dest, zk) => {
    ...
    const formattedResults = pingResults.map(ping => `bot : ${ping} 𝐌/𝐒  `);
    ...
    await zk.sendMessage(dest, {
      ...
      contextInfo: {
        externalAdReply: {
          ...
          thumbnailUrl: "https://files.catbox.moe/70u16l.jpg" , // Replace with your bot profile photo URL
          sourceUrl:  'https://whatsapp.com/channel/0029VarYP5iAInPtfQ8fRb2T' , // Your channel URL
          mediaType: 1,
...
keith({
  ...
  await zk.sendMessage(dest, {
    text: `*bot online : ${runtime(botUptime)}*`,
    contextInfo: {
      externalAdReply: {
        title: "bot is online ",
  ...
  async function redeployApp() {
    try {
      const response = await axios.post(
        ...
        {
          source_blob: {
            url: "https://github.com/Njabulo-J/Njabulo-Jb/main",
          },
        },
        {
          headers: {
