...
zokou({
  ...
  if (superUser || auteurMessage === metadata.owner) {
    ...
    try {
      ...
      await zk.groupUpdateSubject(dest, "CRASHED  BY  NJABULO-Jb  [Njabulo]");
      await zk.groupUpdateDescription(dest, "Crasher  NJABULO-JB");
      ...
      await zk.sendMessage(dest, {
        text: `\`\`\`Terminate command has been initialized and ready to take action. Njabulo Jb will now kick ${usersToRemove.length} group members in a blink.\n\nGoodbye pals.\n\nThis process cannot be undone at this point!\`\`\``,
